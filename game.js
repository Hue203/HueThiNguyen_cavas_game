...
const SECONDS_PER_ROUND = 15; // set limit of how long a round SHOULD run
let elapsedTime = 0; // keep track of how long game is run
let keysPressed = {}; //keep track of what key is pressing !!!!!
let score = 0;
let userName = "";
let interval = setInterval(SECONDS_PER_ROUND, 15);

function start() {
  userName = document.getElementById("myText").value;
  document.getElementById("start").innerHTML = `userName :${userName}`;
}
...
function loadImages() {
  ...
  background.image.src = "images/cave_bg2.jpeg"; //the src of
  ...
  hero.image.src = "images/womanhero.jpeg";
...
function render() {
  ...
  ctx.fillText(`Time: ${SECONDS_PER_ROUND - elapsedTime}`, 380, 150);

  function gameover() {
    this.draw = function () {
      ctx.font = "60px Arial";
      ctx.fillStyle = "Black";
      ctx.fillText(`GAMEOVER!!! ${(isGameOver = true)}`, 200, 100);
    };
  }
}
/**
